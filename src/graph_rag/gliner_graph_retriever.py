...
BM25 is always built during ingestion whenever a corpus exists.
A single ``bm25_weight`` parameter controls the retrieval mix at invoke time:
  - ``bm25_weight == 0`` (default) → dense-only; the BM25 retriever is built
    but not used for this call.
  - ``bm25_weight > 0``  → ensemble (BM25 + dense); ``dense_weight`` is
    implicitly ``1 - bm25_weight``.

This means ``bm25_weight`` can freely be overridden per-call without any
risk of BM25 being unavailable because of how the class was constructed:

    retriever = GLiNERGraphRetriever(
        vectorstore=Chroma(...),
        model_path="urchade/gliner_mediumv2.1",
        labels=["person", "organization"],
        bm25_weight=0.0,    # default: dense-only at invoke time
    )

    # BM25 is always built — this override is always safe:
...
graph construction internally.  BM25 is always built from the child corpus
so that per-call bm25_weight overrides are always available.
    ...
    retriever = GLiNERGraphRetriever(
        ...
        bm25_weight=0.3,    # default weight at invoke time; 0 = dense-only
    )
...
class RetrieverTrace:
    ...
    Attributes:
        ...
        bm25_weight:         Effective BM25 weight used for this call.  ``0.0`` means
                             dense-only was used even though BM25 was available.
...
class EntitySnapshot:
    ...
    Attributes:
        ...
        connected_chunk_ids:       Child IDs directly linked to this entity in the
                                   entity-link graph (depth-1 neighbours that are
                                   chunk nodes, identified via edge_df tail_type).
...
class GLiNERGraphRetriever(BaseRetriever):
    ...
    BM25 is always built from the child corpus during ingestion (``from_documents``,
    ``add_documents``, ``load``).  The ``bm25_weight`` parameter is purely an
    invoke-time routing decision — it controls whether the BM25 retriever is
    actually used for a given call, not whether it is built.  This means any
    per-call ``bm25_weight`` override is always safe regardless of the value
    used at construction time.

    Seed search routing:
      - ``bm25_weight == 0`` → dense-only similarity search for this call.
      - ``bm25_weight > 0``  → EnsembleRetriever (BM25 + dense) for this call;
    ...
    Args:
        ...
        bm25_weight:           Default BM25 weight at invoke time (0–1).
                               ``0`` (default) → dense-only for that call.
                               ``> 0`` → ensemble; ``dense_weight = 1 - bm25_weight``.
                               BM25 is always built during ingestion regardless of
                               this value, so per-call overrides are always safe.
    """
    ...
    def _rebuild_bm25(self) -> None:
        ...
        Called automatically after every ingestion step.  BM25 is always built
        whenever a corpus exists — the ``bm25_weight`` field plays no role here.
        Weight-based routing happens at invoke time in ``_make_ensemble_retriever``.

        Silently skips if the corpus is empty.
        """
        if not self._child_corpus:
            return
        try:
    ...
    def _make_ensemble_retriever(self, k: int, bm25_weight: float):
        """
        Return a fresh EnsembleRetriever for this call, or ``None`` for dense-only.

        A new instance is built per-call so that per-call weight overrides are
        respected without mutating shared state.  BM25 availability is not checked
        here beyond a ``None`` guard — ``_rebuild_bm25`` is responsible for keeping
        ``_bm25_retriever`` populated after every ingestion step.

        Args:
            k:           Number of results each sub-retriever should return.
            bm25_weight: Effective weight for the BM25 leg; ``0`` → dense-only.

        Returns:
            A configured ``EnsembleRetriever``, or ``None`` if ``bm25_weight == 0``
            or ``_bm25_retriever`` is somehow absent.
        """
    ...
    def _parents_from_child_ids(self, child_ids: List[str]) -> List[Document]:
        ...
        for child in self._fetch_children_by_ids(list(child_ids)):
    ...
    def seed_search(
        ...
        """
        Run seed retrieval against child documents and return only results
        that carry a vectorstore-assigned ID.
        ...
        Documents without an ``id`` are filtered out and a warning is emitted,
        because downstream graph traversal depends entirely on child IDs.
        ...
        Returns:
            Child chunk documents carrying ``parent_id`` metadata, all
            guaranteed to have a non-empty ``.id``.
        """
        ...
        if ensemble is not None:
            ...
            results = self.vectorstore.similarity_search(query, k=effective_k)

        # Guard: drop any docs the vectorstore failed to populate with an ID.
        # Without an ID, the doc cannot be matched against the graph, making
        # it useless for traversal and misleading in the trace.
        valid = [doc for doc in results if doc.id]
        if len(valid) < len(results):
            import warnings
            warnings.warn(
                f"seed_search: {len(results) - len(valid)} document(s) returned "
                "by the vectorstore had no assigned ID and were dropped. "
                "Ensure the vectorstore populates Document.id on retrieval.",
                stacklevel=2,
            )
        return valid
    ...
    def load(self) -> "GLiNERGraphRetriever":
        ...
        Rebuilds the BM25 retriever from the persisted child corpus so that
        per-call ``bm25_weight`` overrides work immediately after loading,
        without requiring a fresh call to ``from_documents``.
        ...
        # Always restore child corpus and rebuild BM25 so invoke-time
        # bm25_weight overrides are available immediately after loading.
    ...
    def from_documents(self, documents: List[Document], gliner_batch_size: int = 1) -> None:
        ...
        Splits parents into children, embeds them, runs GLiNER to build the
        knowledge graph, and always rebuilds BM25 from the resulting corpus
        so that per-call ``bm25_weight`` overrides work for any subsequent call.
    ...
    def add_documents(self, documents: List[Document], gliner_batch_size: int = 1) -> None:
        ...
        Rebuilds BM25 from the full updated corpus after each call so that
        per-call ``bm25_weight`` overrides always reflect the latest corpus.
    ...
    def inspect_entity(
        ...
        if self._graph_entity_link is not None and normalised in self._graph_entity_link:
            ...
            # Identify chunk neighbours using edge_df rather than a string
            # heuristic.  Chunk nodes are those whose tail_type is NODE_TYPE_CHUNK
            # in the ENTITY_LINK rows — this is the authoritative source.
            if self._edge_df is not None:
                chunk_tails = set(
                    self._edge_df.loc[
                        (self._edge_df["graph_type"] == GRAPH_TYPE_ENTITY) &
                        (self._edge_df["head"] == normalised) &
                        (self._edge_df["tail_type"] == NODE_TYPE_CHUNK),
                        "tail",
                    ]
                )
    ...
    def get_reachable_triples(
        ...
        appears as a relation target), ``tail_type`` will be an empty string.
        This is a known gap: GLiNER may not always extract an entity mention for
        a node that only appears as a relation tail.
    ...
    def _entity_link_expanded_ids(self, seed_ids: List[str]) -> set[str]:
        """
        Return chunk IDs reachable from seed IDs at exactly 2 hops in the
        entity-link graph.

        The entity-link graph is bipartite: chunk nodes alternate with entity
        nodes (chunk → entity → chunk).  Hop-1 from a seed chunk lands on
        entity nodes; hop-2 lands on *other* chunk nodes that share those
        entities.  The seed IDs themselves are already collected separately,
        so this expansion correctly adds only new chunks without duplicating
        seeds.
        """
    ...
    def _get_relevant_documents(
        ...
        *,
        ...
        seed_ids              = [doc.id for doc in seed_children]  # all guaranteed non-empty by seed_search
        trace.seed_child_ids  = seed_ids
        trace.bm25_weight     = effective_bm25_weight
        ...
        entities                 = self.get_entry_entities(seed_ids)
        trace.candidate_entities = entities
        trace.selected_entities  = entities
